测试提示错误：'Invalid token format', 'message': 'Token must be base64 encoded'
 python 和 golang 在处理 token 的两部分都改成base64 urlsafe的方式

 好的，我们需要修改 Python 测试程序和 Go 服务器代码，使用 base64 URL 安全编码。
...
1. Python 测试程序：
   - 使用 `base64.urlsafe_b64encode` 进行编码
   - 移除末尾的 padding 字符
   - 添加令牌输出以便调试
   - 保持与 URL 参数兼容
...
测试程序会显示加密后的令牌和详细的请求/响应信息，这样更容易诊断问题。您想运行测试吗？

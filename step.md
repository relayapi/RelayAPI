...
## 2024-02-14 更新
1. 更新了 README.md 中的安装说明
   - 添加了 npm 和 pip 包的链接
   - 修改了安装命令
   - 移除了 Docker 安装方式
2. 创建了安装脚本 get_relayapi.sh
   - 支持自动检测系统类型和架构
   - 自动下载对应版本
   - 自动安装到系统目录
3. 创建了编译脚本 build.sh
...
## 下一步工作
1. 测试 get_relayapi.sh 在不同系统上的兼容性
2. 完善 build.sh 的错误处理
3. 添加版本检查和更新功能
4. 补充安装和编译的详细文档

## 2024-02-14 更新 #2
1. 改进了安装脚本 get_relayapi.sh
   - 将所有提示信息改为英文
   - 添加了安装确认功能（默认不安装）
   - 提供了手动安装的提示信息
   - 优化了文件路径处理
...
1. 测试安装脚本在不同系统上的兼容性
2. 验证文件权限设置是否正确
3. 考虑添加更多安装选项（如自定义安装路径）
4. 添加安装脚本的使用说明到文档中

## 2024-02-14 更新 #3
1. 改进了编译脚本 build.sh
   - 添加了严格的错误处理（set -e）
   - 修改编译过程先切换到 server 目录
   - 优化了目录结构和相对路径
   - 添加了详细的错误提示
   - 所有提示信息改为英文
...
## 2024-02-14 更新 #5
1. 修复了编译脚本 build.sh 的配置文件路径
   - 修改配置文件检查位置到 server 目录
   - 更新了配置文件的复制路径
   - 优化了错误提示信息
   - 确保文件路径的正确性
...
## 2024-02-14 更新 #6
1. 修复了编译脚本 build.sh 的目录切换问题
   - 使用 pushd/popd 替代 cd 命令
   - 优化了目录切换的错误处理
   - 隐藏了目录切换的输出信息
   - 确保在错误发生时正确返回原目录
